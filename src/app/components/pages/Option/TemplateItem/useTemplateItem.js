...
import {
  normalizeWithDefaultProps,
  normalizeCheckedItems
} from 'helpers/items';
import { getTemplateItems } from './Edit/service';
import { groupPriceTables } from './helpers';
import _ from 'lodash';

const useEditTemplateItem = () => {
  ...
  const fetchTemplateItems = async () => {
    ...
    try {
      ...
      const normalizedItems = normalizeWithDefaultProps(items);
      const groupedPriceTables = groupPriceTables(normalizedItems);

      setPriceTables(groupedPriceTables);
  ...
  const fetchCheckedTemplateItemsById = async id => {
    try {
      ...
      const normalizedItems = {
        ...normalizeCheckedItems(checkedTemplates),
        ...normalizeWithDefaultProps(_templateItems)
      };
      const groupedPriceTables = groupPriceTables(normalizedItems);
...
export default useEditTemplateItem;
